...
기존 테스트는 가림용 벽과 PlayerMovementController가 활성화된 상태에서 Player Transform을 직접 이동했다. 다음 프레임의 CharacterController 이동이 직접 위치 변경과 경쟁해 카메라 변위가 0~0.39m로 달라졌다. 가림 샘플과 이동 Controller를 끄고 위치 감쇠를 0으로 설정한 뒤, 가상 카메라 상태를 재계산해 3m Target 이동을 2.5m 이상 즉시 추적하는지 검증하도록 수정했다.
